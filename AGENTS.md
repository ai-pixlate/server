...
- **branch 생성 전에 사용자에게 묻는다.** 브랜치 유형(`feature/*` · `fix/*`)과 이름을 제안하고,
  사용자가 승인한 뒤에 만든다.
- **commit · push · PR 생성은 사용자가 요청할 때만 한다.** 변경 범위와 커밋 메시지 안을 보여주고
  의사를 묻는 데까지만 하고 멈춘다. 먼저 실행하지 않는다.
- **문서에 없는 것은 코드에서 정하지 않는다.** 엔드포인트 · 필드 · enum 값 · 에러 코드 · 스키마가
  정본 문서에 없으면 임의로 채우지 말고 멈춰서 사용자에게 묻는다.

| 사안 | 정본 |
| --- | --- |
| API 계약 — 엔드포인트 · 요청/응답 · 에러 | `docs/openapi.yaml` · `docs/README.md` |
| DB 스키마 | `migrations/versions/` (`db/schema.sql`은 참고 스냅샷) |
| 서비스 흐름 · BE ↔ AI 경계 | `README.md` |
| AI 파이프라인 | `docs/ai/` — 아래 절 |
