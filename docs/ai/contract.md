...
- ③ 병합 보정(`llm_assist`)의 LLM 호출·응답 검증 실패도 원본 계약에 없음. 같은 항목이 **미정**(10장, `open-questions.md` #37).
...
- 미확정 사항 → `open-questions.md`. 원본 10.2 항목(제품명/효능 구분, 판정 항목 목록·경계, 규제·채널 정책 데이터, `section_verdict` 허용값 매핑, RAG 프롬프트) [계약 10장]에 더해 이 요약에서 드러난 **계약 공백 8건**:
  ...
  - ③ `llm_assist` 호출·응답 검증 실패의 대체 처리 허용 여부 · 오류·경고 코드 · 재시도 조건 (8장)
  - 텍스트가 빈 원시 영역만으로 이루어진 블록(`source_ko` 빈 문자열)의 저장 허용 여부와 `role` (2장 · 2.5)
