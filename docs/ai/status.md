...
| ② 텍스트 추출 | `pipeline/stages/ocr.py` · `pipeline/jsonio.py` | **구현 완료(4,000px 이하)** — PaddleOCR 어댑터 · 영역 변환(폴리곤 반올림·섹션 범위 자르기 · bbox) · 실패 처리 · CLI 실행 구조(D8). 합성·계약 테스트 통과, 실제 엔진 통합 테스트는 OCR 환경에서 통과. 4,000px 초과는 거부(`NotImplementedError`, 종료 코드 3) | 채점 명세 v2 · 품질 표본 정답 동결 → v1 전체 112섹션 기준선 → 타일링(#31 · #32) · `retryable` 세분(#33, BE) |
