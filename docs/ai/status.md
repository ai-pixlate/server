...
| 단계 | 파일 | 상태 | 남은 것 |
|---|---|---|---|
| ① 섹션 분해 | `pipeline/stages/section_split.py` | 뼈대 — `open_source()`(IMAGE_OPEN_FAILED) · `crop_sections()` 동작, **경계 결정 미구현** | 배경색 전환 경계 규칙(임계값 등 세부가 정본에 없으면 `open-questions.md`에 올린다) · VLM 경계 선택(모델명 #21 · 실패 처리 #25 BE 합의) |
