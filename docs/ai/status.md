...
1. **① 확정 후보 반영과 남은 실험** — 3차 기준값 실행 채점: 정답 검출 68/70, 누락 2, 오절단 10 + 알려진 한계 4. 사용자 "확정" 시 확정 후보 3개를 config·`pipeline.md` 3절에 반영. 다음 실험은 각각 독립으로: #27 목록 규칙(정의 확정 후 34장 전체·VLM 포함 채점) · #28 프롬프트 개정 · `blank_row_std` 단독 비교. 빈 구간 병합 재실행이 실제 표본에서 어떤 경계를 바꾸는지는 다음 실측에서 확인.
