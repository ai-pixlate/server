...
2. **① 후속 개선(별도 PR)** — ①은 4차 실측(34장, 정답 검출 69/70, 오절단 6 + 알려진 한계 4)까지 검증하고 통합했다. 남은 것은 (a) #28 프롬프트 실험 전에 정답 목록의 채점 기준 보완(`dev.md` 5절 대체 위치 규칙), (b) #28 프롬프트 실험(현행 비교군 포함 3조건 × 3회, VLM 호출 이미지 20장), (c) #27 목록 규칙은 채택 조건 충족 전까지 보류. 알려진 한계: FAQ 항목·제목 띠 일부 오절단, VLM 비결정성(실행마다 오가는 경계). 파라미터 탐색은 종료.
