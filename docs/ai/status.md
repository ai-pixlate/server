...
1. **① 남은 실험** — 4차 기준값 실행 채점: 정답 검출 69/70, 누락 1, 오절단 6 + 알려진 한계 4(3차 대비 개선은 전부 VLM 비결정성, 코드 효과 아님). 파라미터 탐색은 종료(추가 조정의 이득을 확인하지 못함). (0) 정답 목록의 채점 기준을 먼저 보완(`dev.md` 5절 대체 위치 규칙, 예: GS-01_002 24353/24460). (1) #28 프롬프트 실험 — 현행 비교군 포함 3조건 × 3회, 대상은 VLM 호출 이미지 20장. (2) #27은 채택 조건 충족 전까지 보류. 완료: `blank_row_std` 6.0 vs 8.0 단독 비교(4차).
