...
| ② 텍스트 추출 | `pipeline/stages/ocr.py` · `pipeline/jsonio.py` | 착수 준비 — 준비 작업 A 완료(경로 기준·버전 2 · `convert-split` · `freeze-input` · `verify-input` · `run.json` 커밋 해시). `ocr.run`은 미구현 | B `main()`의 `AnalyzeError` 처리·D8 실행 구조 → C `ocr.run`(4,000px 이하, D6 · #34 해결 후) → D 타일링(#31 확정 후) · 중복 정리(#32 절차·잠정 값 승인 후) · `requirements-ocr.txt` 버전 고정 |
...
1. **② OCR** — 착수 계획(2026-09-23) 확정분: D2 경로 기준·버전 2(구현 완료) · D6 반환 영역 보존(조건부, `pipeline.md` 7절) · D8 실행 구조 · D9 채점 명세 v1 동결(PoC 레포 보관, 1인 검수). 미결 #31~#34 등록(구현 승인 아님). 다음: (a) 로컬 1a — 환경 설치 · v1 섹션 목록 · **품질 표본 후보 선정 후 그 밖의 섹션으로** PaddleOCR 단독 프로브(배열 대응 · 임계값 0/기본 · 빈 텍스트 · 점수 분포 · 예외 클래스) (b) 로컬 1b — `freeze-input`으로 OCR 고정 입력 v1 생성(4절) (c) B · C · D 순 구현(1절 ② 행). 품질 표본은 정답 v1과 채점 명세 v1을 동결하기 전까지 어떤 OCR에도 넣지 않는다. 완료 상태 표기: 구현 완료 / 기준선 측정 완료, 품질 승인 대기(모든 섹션 `ok`) · 기준선 부분 측정: 성공 N / 실패 M / 미실행 K / 품질 승인(사용자).
