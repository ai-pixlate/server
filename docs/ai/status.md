...
| 단계 | 파일 | 상태 | 남은 것 |
|---|---|---|---|
| ① 섹션 분해 | `pipeline/stages/section_split.py` · `pipeline/vlm.py` | 로컬 검증 + 1차 실측(34장) 반영 — 배경다움 조건 · 빈 구간 병합 · 강도 기준 최소 높이 · 폭 기준 VLM 입력·창 분할 · 진단 기록. 테스트 26개 통과, `synthetic_01` 일치 | 2차 실측으로 파라미터 확정(#26) · 후기·FAQ 목록 항목별 절단(유형 B) 규칙 결정 · VLM 실패 처리(#25 BE 합의) |
...
1. **② OCR** — 로컬 CPU에서 되므로 다음 착수. `requirements-ocr.txt` 검증을 겸한다. 완료 기준: `samples/local/` 표본에서 `ocr/<key>.json` + 오버레이 확인, 4,000px 초과 섹션 분할·복원 테스트.
2. **① 글자 잘림 수정 검증(이 PR 병합 전)** — 색 단계만: `--set section.long_section_px=999999`로 34장 실행해 r4/base의 색 경계(`color_candidates` accepted)와 비교, 이동(`snapped`)·기각(`no_blank_row`) 목록과 이동 거리, GS-02_014 9234·8626의 새 위치를 확대 이미지로 확인(제목이 위·아래 어느 섹션에 속하는지 포함). 그다음 전체 실행(VLM 포함)으로 정답 채점이 4차 기준에서 나빠지지 않는지, 새 글자 잘림이 없는지(경계 ±1 균일 행 검사는 검토 대상 선별용) 확인. 통과하면 그 결과를 OCR 고정 입력 v1로 보관하고 4차 결과는 비교용으로 유지.
3. **① 후속 개선(별도 PR)** — ①은 4차 실측(34장, 정답 검출 69/70, 오절단 6 + 알려진 한계 4)까지 검증하고 통합했다. 남은 것은 (a) #28 프롬프트 실험 전에 정답 목록의 채점 기준 보완(`dev.md` 5절 대체 위치 규칙), (b) #28 프롬프트 실험(현행 비교군 포함 3조건 × 3회, VLM 호출 이미지 20장), (c) #27 목록 규칙은 채택 조건 충족 전까지 보류. 알려진 한계: FAQ 항목·제목 띠 일부 오절단, VLM 비결정성(실행마다 오가는 경계). 파라미터 탐색은 종료.
