...
1. **① 남은 실험(순서 확정)** — 3차 기준값 실행 채점: 정답 검출 68/70, 누락 2, 오절단 10 + 알려진 한계 4. (1) `blank_row_std` 6.0 vs 8.0 단독 비교(프롬프트·다른 설정 고정. 색 경계 차이 → VLM 입력이 같은 이미지는 `--vlm-replay`로 보정 차이 → 구간이 달라진 이미지는 실제 호출로 전체 품질). 같은 실행에서 VLM 보정 뒤 빈 구간 병합이 실제 표본에서 바꾸는 경계 확인. (2) #28 프롬프트 개정(오절단 억제와 독립 소제목 포함 지시를 함께). (3) #27은 분석만(띠 짝짓기·분포 표·규칙 적용 시 제거될 경계 목록), 구현 보류.
